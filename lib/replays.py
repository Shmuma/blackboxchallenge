...
class ReplayGenerator:
    ...
    def reset_bbox(self):
        log.info("ReplayGenerator: bbox resetted at time step {step}".format(
    ...
    def next_batch(self):
        ...
        while len(batch) < self.batch_size:
            state = features.transform(infra.bbox.get_state(), rewards_history, actions_history, infra.bbox.get_time())
            rewards_states = infra.dig_all_actions(self.score)
            rewards, next_states = zip(*rewards_states)
            next_states = map(lambda (idx, state): features.transform(state, rewards_history + [rewards[idx]],
                                                                      actions_history + [idx], infra.bbox.get_time()+1),
            ...
            rewards_history = features.push_history(rewards_history, rewards[action])
            actions_history = features.push_history(actions_history, action)
